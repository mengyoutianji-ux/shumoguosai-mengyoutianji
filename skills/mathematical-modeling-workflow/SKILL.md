...
1. Read [`AGENTS.md`](../../AGENTS.md), the public `../../config/workflow.yml`, the optional machine-local `../../config/workflow.local.yml` when it exists, [rule versions and conflict handling](../../docs/规则版本与冲突处理.md), the [workflow overview](../../docs/工作流总览.md), and [data, model, and result reasonableness](../../docs/数据模型与结果合理性规范.md). The local file may override only `project.default_root` and `python.preferred_executable`.
...
- For material inventory, ambiguity handling, preprocessing decisions, model transitions, custom thresholds, assumption diagnostics, model comparison, fitting, optimization, uncertainty, or near-perfect fit, read [modeling and statistics](../../docs/建模与统计规范.md) and [data, model, and result reasonableness](../../docs/数据模型与结果合理性规范.md).
...
- Before committing or publishing repository content, read [publication boundary](../../docs/发布边界.md). Never stage or publish `config/workflow.local.yml`.
...
- Explain why, how, and validation evidence for every consequential preprocessing rule, model structure, custom metric, and threshold. Determine data-driven thresholds inside training data only.
- Explain how each model is built, why it fits the data, and what verified advantage it has; after fitting, separate model selection, assumption diagnostics, and sensitivity analysis.
...
- Resolve the configured Python with `../../scripts/get_workflow_python.ps1`, which prefers the ignored machine-local override and otherwise reads the public config. Confirm it exists; do not call system Python or install packages from the network unless the user separately authorizes it.
